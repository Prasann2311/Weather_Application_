    ...
    private static void displayWeatherData(double latitude, double longitude){
        try{
            ...
            String url = "https://api.open-meteo.com/v1/forecast?latitude="
                    + latitude + "&longitude="
                    + longitude + "&current=temperature_2m,relative_humidity_2m,is_day,wind_speed_10m";
            HttpURLConnection apiConnection = fetchApiResponse(url);
            https://api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&current=temperature_2m,relative_humidity_2m,is_day,wind_speed_10m
            // check for response status
            // 200 - means that the connection was a success
            if(apiConnection.getResponseCode() != 200){
                System.out.println("Error: Could not connect to API");
